...
export const advisingAPI = {
  ...
  // Get messages with advisor
  getMessages: async (advisorId: string, page?: number, limit?: number) => {
    const response = await apiClient.get(`/advising/messages/${advisorId}`, {
      params: { page, limit },
    });
    return response.data;
  },
  ...
  sendMessage: async (advisorId: string, content: string, attachments?: File[]) => {
