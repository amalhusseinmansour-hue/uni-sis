...
class DynamicReport extends Model
{
    ...
    protected function executeRawQuery(array $params): array
    {
        ...
        // SECURITY: Use Laravel's parameter binding instead of string replacement
        // This prevents SQL injection attacks
        $bindings = [];

        foreach ($params as $key => $value) {
            // Check if the parameter is used in the query
            if (strpos($query, ":{$key}") !== false) {
                $bindings[$key] = $value;
            }
        }

        // Use DB::select with named bindings for security
    ...
    protected function executeModelQuery(array $params): array
    {
        ...
        // Apply sorting - SECURITY: Validate sort direction
        if ($this->sorting) {
            foreach ($this->sorting as $sort) {
                ...
                // SECURITY: Validate field name - only allow alphanumeric and underscore
                $field = preg_replace('/[^a-zA-Z0-9_.]/', '', $sort['field']);
                if (!empty($field)) {
    ...
    {
        // SECURITY: Validate procedure name - only allow alphanumeric and underscore
        $procedure = preg_replace('/[^a-zA-Z0-9_]/', '', $this->data_source);
        if (empty($procedure)) {
