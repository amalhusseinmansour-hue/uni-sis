...
class StudentIdCardService
{
    ...
    public function getDigitalIdCard(Student $student): array
    {
        ...
        return [
            ...
            'validity' => [
                ...
                'expiry_date' => $this->getExpiryDate($semester),
            ],
            'verification' => [
            ...
            'university' => [
                'name_en' => Setting::getValue('university_name_en', 'University'),
                'name_ar' => Setting::getValue('university_name_ar', 'الجامعة'),
                'logo' => Setting::getValue('university_logo'),
            ],
        ];
    }

    /**
     * Get expiry date based on settings
     */
    protected function getExpiryDate(?Semester $semester): string
    {
        if ($semester?->end_date) {
            return $semester->end_date->format('Y-m-d');
        }

        $validityMonths = (int) Setting::getValue('id_card_validity_months', 6);
        return now()->addMonths($validityMonths)->format('Y-m-d');
    }
    ...
    public function generateQRData(Student $student): string
    {
        $data = [
            ...
            'status' => $student->status,
            'valid_until' => $this->getExpiryDate(Semester::getCurrentSemester()),
    ...
    public function generateIdCardPdf(Student $student): string
    {
        ...
        $universitySettings = Setting::getUniversitySettings();
        $idCardSettings = Setting::getIdCardSettings();
        ...
        $pdfData = [
            ...
            'university_name' => $universitySettings['university_name_en'] ?? 'University',
            'university_name_ar' => $universitySettings['university_name_ar'] ?? 'الجامعة',
            'university_logo' => $universitySettings['university_logo'] ?? null,
            'background_color' => $idCardSettings['id_card_background_color'] ?? '#1e293b',
            'accent_color' => $idCardSettings['id_card_accent_color'] ?? '#eab308',
        ];
    ...
    public function generateBulkIdCardsPdf(array $studentIds): string
    {
        ...
        foreach ($students as $student) {
            ...
            $cardsData[] = [
                ...
                'validity' => [
                    'issue_date' => now()->format('Y-m-d'),
                    'expiry_date' => $this->getExpiryDate(Semester::getCurrentSemester()),
                ],
            ];
        }
