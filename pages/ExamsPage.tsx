...
const ExamsPage: React.FC<ExamsPageProps> = ({ lang }) => {
  ...
  // Default exam data (fallback)
  const defaultUpcomingExams = [
    {
      id: '1',
      course: 'CS101',
      title: lang === 'ar' ? 'مقدمة في علوم الحاسوب' : 'Intro to Computer Science',
      type: 'final',
      date: '2024-12-20',
      time: '09:00 - 11:00',
      duration: 120,
      ...
      topics: ['Arrays', 'Loops', 'Functions', 'OOP Basics'],
      materials: lang === 'ar' ? 'الفصول 1-8' : 'Chapters 1-8',
      notes: lang === 'ar' ? 'يُسمح بالآلة الحاسبة' : 'Calculator allowed',
    },
    {
      id: '2',
      course: 'MATH201',
      title: lang === 'ar' ? 'تفاضل وتكامل ٢' : 'Calculus II',
      type: 'final',
      date: '2024-12-22',
      time: '14:00 - 16:00',
      duration: 120,
      ...
      materials: lang === 'ar' ? 'الفصول 5-10' : 'Chapters 5-10',
      notes: lang === 'ar' ? 'يُسمح بجدول الصيغ' : 'Formula sheet allowed',
    },
    {
      id: '3',
      course: 'CS201',
      title: lang === 'ar' ? 'هياكل البيانات' : 'Data Structures',
      type: 'final',
      date: '2024-12-25',
      time: '09:00 - 11:00',
      duration: 120,
      ...
      topics: ['Trees', 'Graphs', 'Hash Tables', 'Sorting'],
      materials: lang === 'ar' ? 'جميع المحتوى' : 'All content',
      notes: '',
    },
  ];
  ...
  const defaultExamResults = [
    { course: 'CS101', midterm: 85, quizzes: 92, assignments: 88, final: null, total: null },
    { course: 'MATH201', midterm: 78, quizzes: 85, assignments: 90, final: null, total: null },
    { course: 'CS201', midterm: 88, quizzes: 90, assignments: 95, final: null, total: null },
    { course: 'ENG102', midterm: 82, quizzes: 88, assignments: 85, final: null, total: null },
  ];

  // Use API data or fallback
  const displayUpcomingExams = upcomingExams.length > 0 ? upcomingExams : defaultUpcomingExams;
  const displayPastExams = pastExams.length > 0 ? pastExams : defaultPastExams;
  const displayExamResults = examResults.length > 0 ? examResults : defaultExamResults;
          ...
          >
            ...
            <div className="p-5">
              <div className="flex items-start gap-4">
                {/* Date Box */}
                <div className={`w-20 h-20 rounded-xl flex flex-col items-center justify-center shadow-lg ${
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    <span className="px-2.5 py-1 bg-slate-100 dark:bg-slate-700 rounded-lg text-xs font-mono font-bold text-slate-700 dark:text-slate-300">
                      {exam.course}
                    </span>
                    ...
                    {daysUntil <= 3 && (
                      <span className="px-2.5 py-1 bg-red-100 dark:bg-red-900/50 text-red-700 dark:text-red-300 text-xs rounded-lg font-medium animate-pulse">
                        {lang === 'ar' ? `${daysUntil} أيام متبقية` : `${daysUntil} days left`}
                      </span>
                    )}
                  </div>
                  <h3 className="font-bold text-slate-800 dark:text-slate-100 text-lg">{exam.title}</h3>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-3">
                    ...
                    <div className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-400">
                      ...
                      {exam.duration} {lang === 'ar' ? 'دقيقة' : 'min'}
                    </div>
                    <div className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-400">
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
        ...
        <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex gap-3 flex-wrap">
            <select
              ...
              className="px-3 py-2 bg-white/20 border border-white/30 rounded-lg text-white text-sm focus:ring-2 focus:ring-white/50 focus:outline-none"
            >
      ...
      {/* Tabs */}
      <div className="flex items-center gap-2 bg-slate-100 dark:bg-slate-800 rounded-xl p-1.5 w-fit">
        <button
        ...
        >
          ...
          {displayUpcomingExams.length > 0 && (
            <span className="px-1.5 py-0.5 bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-400 text-xs rounded-full">
