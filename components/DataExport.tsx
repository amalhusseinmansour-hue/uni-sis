            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <div className="relative">
                    <Calendar className={`absolute top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 ${isRTL ? 'right-3' : 'left-3'}`} />
                    <input
                      type="date"
                      value={dateFrom}
                      onChange={(e) => setDateFrom(e.target.value)}
                      className={`w-full py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${isRTL ? 'pr-10 pl-3' : 'pl-10 pr-3'}`}
                    />
                  </div>
                </div>
                <div>
    ...
    <div className="relative">
      ...
      {showMenu && (
        <>
          ...
          <div className={`absolute top-full mt-1 bg-white rounded-lg shadow-lg border border-slate-200 py-1 z-20 min-w-[160px] ${lang === 'ar' ? 'left-0' : 'right-0'}`}>
            {(Object.keys(formatConfig) as ExportFormat[]).map((format) => {
              const config = formatConfig[format];
              const Icon = config.icon;
              return (
